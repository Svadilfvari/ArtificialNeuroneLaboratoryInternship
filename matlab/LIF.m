function [Vtemps,AMPFS] = LIF(DC,Fe,Fsp,Vpp)
% LIF  Spike train of the Leaky Integrate-and-Fire neuron model.
%   [Vtemps, AMPFS] = LIF(DC, Fe, Fsp, Vpp)
%   DC  : fraction of the period spent charging (Ton = DC*T)
%   Fe  : samples per ms (time step Te = 1/Fe ms)
%   Fsp : spike frequency from the polynomial fit (Hz)
%   Vpp : spike amplitude (mV)
%   Vtemps : time vector (ms), AMPFS : membrane potential (mV)
%   Each spike is A1*exp(Lambda1*t)+B1 (charge) followed by a reset;
%   the period is scaled by an empirical constant (3.40) calibrated on the
%   Cadence transient at Iex = 30 pA.
