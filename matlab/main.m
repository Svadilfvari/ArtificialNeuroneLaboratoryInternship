%% main.m - Matlab model of a Leaky Integrate-and-Fire (LIF) neuron
% L3 research internship, GeePs laboratory (Sorbonne Universite / CentraleSupelec), 2021-2022
% Authors: Edouard David, Marc Zhan - Supervisors: A. Benlarbi-Delai, P. M. Ferreira
%
% Pipeline:
%   1. read the Cadence operating points (Iex, Prms, fspike) of the FS eNeuron
%   2. fit fspike(Iex) and Prms(Iex) with 15th-order polynomials (polyfit)
%   3. compute RMS / relative errors of the fits
%   4. generate the spike train for a given Iex with LIF.m
%   5. overlay it on the Cadence transient simulated at Iex = 30 pA
%
% Run from any folder: the data files are read from ../data relative to this script.

dataDir = fullfile(fileparts(mfilename('fullpath')), '..', 'data');
csvFile = fullfile(dataDir, 'TB_Ferreira2020_FS_MLneuron_PLS_FoM.csv');

%% recuparation des données à partir du fichier csv
IEX = readtable(csvFile,'Range','A3:A311');
 

Iex=table2array(IEX);% transformer le tableau en un vecteur

PRMS = readtable(csvFile,'Range','B3:B311');

Prms=table2array(PRMS);%transformer le tableau en un vecteur

FSPIKE = readtable(csvFile,'Range','C3:C311');
...
Iex1= (3E-9).*10^7;% = 0.03 in the fit's nA units, i.e. Iex = 30 pA
...
Iex2= (1E-9).*10^7;% = 0.01 in the fit's nA units, i.e. Iex = 10 pA (below the data range)
...
Vtemps_cadence =readtable(fullfile(dataDir,'Vout_LIF_30p.csv'),'Range','A2:A12823');

Vtemps_cadence =table2array(Vtemps_cadence );% transformer le tableau en un vecteur

Vout_cadence =readtable(fullfile(dataDir,'Vout_LIF_30p.csv'),'Range','B2:B12823');
Vout_cadence =table2array(Vout_cadence );% tension de sortie Cadence (V)
...
% NOTE: s and N are carried over from the previous blocks, and the Cadence trace
% (0.1 us step) and AMPFS (0.5 us step) use different time grids, so this
% index-by-index comparison is only indicative. See README > Reproducibility notes.
T1=0.318;
T2=0.374;
i1= T1*2000;
i2=T2*2000;
